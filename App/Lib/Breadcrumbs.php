...
class Breadcrumbs {
    ...
    public $template = [
        'linked'        => '<li>{{link}}</li>',
        'not-linked'    => '<li><a href="javascript:void(0);">{{label}}</a></li>'
    ];
    ...
    public function render()
    {
        try {
            ...
            foreach ($this->crumbs as $key => $crumb) {
                if ($crumb['linked']) {
                    $label = (is_null($this->translate)) ? $crumb['label'] : $this->translate->_($crumb['label']);
                    if(strpos($crumb['link'], ':')){
                        $link = array('for' => $crumb['link'] );
                    }else{
                        $link = $crumb['link'];
                    }
                    $output .= str_replace('{{link}}', 
                        \Phalcon\Tag::linkTo( array($link, $label, 'onClick' => '_gaq.push([\'_trackEvent\', \'Links\', \'Click\', \'Breadcrumb '.$label.'\']);' )),
